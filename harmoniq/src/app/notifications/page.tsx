...
import { Calendar, CheckCheck, MessageCircle, Music, Trophy } from "lucide-react";
import { NOTIFICATIONS, type NotificationKind } from "@/lib/data";
...
export default function NotificationsPage() {
  ...
  const Row = (n: (typeof items)[number]) => (
    <div
      key={n.id}
      className="flex gap-4 rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900"
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-2 text-xs font-bold text-slate-500 dark:text-slate-500">{n.timestamp}</p>
      </div>
      <div className="flex shrink-0 items-start pt-1">
        {n.unread ? (
          <span className="h-2.5 w-2.5 rounded-full bg-teal-600 dark:bg-teal-400" title="Unread" />
        ) : (
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          type="button"
          onClick={markAllRead}
          className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-extrabold text-slate-800 transition hover:border-teal-200 hover:bg-teal-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:border-teal-800"
        >
          <CheckCheck className="h-4 w-4 text-teal-700 dark:text-teal-400" />
          Mark all as read
        </button>
      </div>
